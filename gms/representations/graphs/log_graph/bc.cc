...
void PBFS(const CSRGraph &g, NodeId source, pvector<NodeId> &path_counts,
...
pvector<ScoreT> Brandes(const CSRGraph &g, SourcePicker<CSRGraph> &sp,
						NodeId num_iters) {
	#if PRINT_INFO
		Timer t;
		t.Start();
...
bool BCVerifier(const CSRGraph &g, SourcePicker<CSRGraph> &sp, NodeId num_iters,
...
int main(int argc, char* argv[]) {
  ...
  auto BCBound =
	[&sp, &cli] (const CSRGraph &g) { return Brandes(g, sp, cli.num_iters()); };
  SourcePicker<CSRGraph> vsp(g, cli.start_vertex());
  auto VerifierBound = [&vsp, &cli] (const CSRGraph &g,
