...
#ifndef GRAPHSETS_COMPRESSED_VARINT_BYTE_BASED_H
#define GRAPHSETS_COMPRESSED_VARINT_BYTE_BASED_H

#include "coders-utils/varint_utils.h"


class VarintByteBasedGraph {
public:
...
public:
    ...
    VarintByteBasedGraph(int64_t num_nodes, int64_t num_edges, bool directed,
            uint64_t* new_offsets_out,
            uint64_t* new_offsets_in,
            unsigned char* new_adj_data_out,
            unsigned char* new_adj_data_in) {
    ...
    VarintByteBasedGraph(const VarintByteBasedGraph &) = delete;
    VarintByteBasedGraph &operator=(const VarintByteBasedGraph &) = delete;

    VarintByteBasedGraph(VarintByteBasedGraph &&g) :
    ...
    int64_t out_degree(NodeId v) const {
        uint64_t nr_neighs = 0;
        unsigned char* pos = &new_adj_data_out[new_offsets_out[v]];
    ...
    int64_t in_degree(NodeId v) const {
        uint64_t nr_neighs = 0;
        unsigned char* pos = &new_adj_data_in[new_offsets_in[v]];
        fromVarint(pos, &nr_neighs);
