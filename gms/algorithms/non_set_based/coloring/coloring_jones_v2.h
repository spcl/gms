...
namespace GMS::Coloring::JonesV2 {
...
template <class CGraph>
int32_t sequential_coloring_ido(const CGraph& g, std::vector<int32_t> &coloring, const std::vector<NodeId> &color_queue,
                                std::vector<int32_t> &n_wait, const node_queue_list &send_queues,
                                std::vector<ready_queue> &ready_queues, const int64_t part_max_size,
                                std::vector<bool> &color_palette) {
    int32_t max_color = 0;
    ...
    for (size_t i = 0; i < n_to_color; ++i) {
        ...
        for (NodeId w : g.out_neigh(v)) {
            ...
            w_color = coloring[w];
            if (w_color > 0) {
                ++num_colored_neighbors;
            }
        }

        incidence_degree[i] = num_colored_neighbors;
        rev_map[v] = i;
        max_deg_heap[i] = i;
    }
    ...
    for (size_t i = 0; i < n_to_color; ++i) {
        if (modified) {
            std::make_heap(max_deg_heap.begin(), max_deg_heap.end(), [&](const size_t a, const size_t b) -> bool {
                return incidence_degree[a] < incidence_degree[b];
            });
            modified = false;
        }
...
template <class CGraph>
int32_t sequential_coloring_sdo(const CGraph& g, std::vector<int32_t> &coloring, const std::vector<NodeId> &color_queue,
                                std::vector<int32_t> &n_wait, const node_queue_list &send_queues,
                                std::vector<ready_queue> &ready_queues, const int64_t part_max_size,
                                std::vector<bool> &color_palette) {
    ...
    struct idx_and_degree {
        ...
        idx_and_degree(size_t idx, int64_t d) : index1(idx), degree(d) {}
        bool operator <(const idx_and_degree &other) {
            return degree > other.degree; // Sort descending by degree
        }
    };
    ...
    std::vector<idx_and_degree> degree_ordering;
    degree_ordering.reserve(n_to_color);

    for (size_t i = 0; i < n_to_color; ++i) {
        NodeId v = color_queue[i];
        degree_ordering.emplace_back(i, g.out_degree(v));
    }

    std::sort(degree_ordering.begin(), degree_ordering.end());

    // Color a vertex of maximal degree
    NodeId v = color_queue[degree_ordering[0].index1];
    int32_t color = pick_lowest_consistent_color(g, coloring, v, color_palette);
    max_color = std::max(max_color, color);
    notify_threads(send_queues[v], n_wait, ready_queues, part_max_size);

    // Order the rest of the vertices into a heap by decreasing saturation degree
    struct idx_and_saturation {
        size_t index2;
        int64_t saturation;

        idx_and_saturation(size_t idx)
 	  : index2(idx), saturation(0) {}

        bool operator<(const idx_and_saturation &other) {
	    return saturation < other.saturation;
        }

        void update_saturation(const CGraph &g, const std::vector<int32_t> &coloring,
			       const std::vector<NodeId> &color_queue) {
	    std::unordered_set<int32_t> distinct_colors;
	    NodeId w = color_queue[index2];
 	    for (NodeId o : g.out_neigh(w)) {
		int32_t v_color = coloring[o];
	        if (v_color > 0)
		    distinct_colors.insert(v_color);
	    }
	    if (saturation < distinct_colors.size())
	        saturation = distinct_colors.size();
        }
    };
    using idx_and_sat_ptr = std::shared_ptr<idx_and_saturation>;

    std::vector<idx_and_sat_ptr> saturation_ordering;
    saturation_ordering.reserve(n_to_color - 1); // Omit the vertex already colored
    std::unordered_map<NodeId, idx_and_sat_ptr> rev_map;
    for (size_t i = 0; i < n_to_color; ++i) {
        NodeId w = color_queue[i];
        if (w != v) {
	    idx_and_sat_ptr idx {new idx_and_saturation(i)};
	    saturation_ordering.emplace_back(idx);
	    rev_map[w] = idx;
	}
    }

    for (NodeId w : g.out_neigh(v)) {
        idx_and_sat_ptr idx = rev_map[w];
	if (idx)
	    idx->update_saturation(g, coloring, color_queue);
    }

    // Color the rest of the verticies based on their saturation degrees
    bool modified = true;
    for (size_t i = 0; i < n_to_color - 1 && !saturation_ordering.empty(); ++i) {
        if (modified) {
	    std::make_heap(saturation_ordering.begin(), saturation_ordering.end(), [](const idx_and_sat_ptr &a, const idx_and_sat_ptr &b) {
                return a->saturation < b->saturation;
	    });
	    modified = false;
        }

	// Pick vertex with the largest saturation degree value
        NodeId w = color_queue[saturation_ordering[0]->index2];
	rev_map[w].reset();
	rev_map[w] = nullptr;
        // Remove the vertex from the heap
        std::pop_heap(saturation_ordering.begin(), saturation_ordering.end());
        saturation_ordering.pop_back();

	// Pick a color for the vertex
        int32_t color = pick_lowest_consistent_color(g, coloring, w, color_palette);
        max_color = std::max(max_color, color); // Possibly modify max_color if new is higher
        notify_threads(send_queues[w], n_wait, ready_queues, part_max_size);

	// For each neighbour, update saturation
        for (NodeId o : g.out_neigh(w)) {
	    idx_and_sat_ptr it = rev_map[o];
	    if (it) {
	        int64_t old_saturation = it->saturation;
		it->update_saturation(g, coloring, color_queue);
		if (it->saturation > old_saturation)
		    modified = true;
	    }
        }
    }
...
template <class CGraph>
int32_t graph_coloring_jones(const CGraph &g, std::vector<int32_t> &coloring) {
//    DetailTimer timer;

    const seq_coloring_func<CGraph> seq_color = &sequential_coloring_sdo;
...
//            timer.endPhase("par_partition");
        ...
        num_colors = seq_color(g, coloring, color_queue, n_wait, send_queues, ready_queues, part_max_size, color_palette);
        size_t n_colored = color_queue.size();
        color_queue.clear();

        while (n_colored < shared_vertices_count) {
            in_queue.dequeue(color_queue);
            int32_t cur_max_color = seq_color(g, coloring, color_queue, n_wait, send_queues, ready_queues, part_max_size, color_palette);
        ...
        int32_t cur_max_color = seq_color(g, coloring, local_vertices, n_wait, send_queues, ready_queues, part_max_size, color_palette);
        num_colors = std::max(num_colors, cur_max_color);
    }
